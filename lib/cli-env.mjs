...
export function loadWalletEnv({ proxyOverride = null, debugLogger = () => {} } = {}) {
	...
	// Both lines are debug-level — they match trunk's `log()` (DEBUG-gated)
	// behavior so opting into mpp-remote doesn't add stderr noise by default.
	debugLogger(`wallet: ${account.address} (${privyAddress ? 'privy' : 'local'})`);
	if (proxy) debugLogger(`using proxy: ${proxy}`);
